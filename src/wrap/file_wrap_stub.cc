...
// TODO(yusukes): We no longer have a reason to rename __wrap_XXX to
// wrap_XXX_func. Stop doing that.
#define wrap_preadv_func __wrap_preadv
#define wrap_pwritev_func __wrap_pwritev
...
extern "C" int __wrap_ppoll(struct pollfd* fds, nfds_t nfds, int timeout,
                            const struct timespec* timeout_ts,
                            const sigset_t* sigmask) {
  // TODO(crbug.com/241955): Stringify |fds|?
  ARC_STRACE_ENTER("ppoll", "%p, %lld, %d, %p, %p",
                     fds, static_cast<int64_t>(nfds), timeout,
                     timeout_ts, sigmask);
  DANGERF("ppoll: fds=%p nfds=%lld timeout=%d timeout_ts=%p sigmask=%p",
                     fds, static_cast<int64_t>(nfds), timeout,
                     timeout_ts, sigmask);
  ALOG_ASSERT(0);
  errno = ENOSYS;
...
extern "C" ssize_t wrap_preadv_func(int fd, const struct iovec* iov, int iovcnt,
                                    off_t offset) {
  ARC_STRACE_ENTER_FD("preadv", "%d, %p, %d, %lld",
                        fd, iov, iovcnt, static_cast<int64_t>(offset));
  DANGERF("preadv: fd=%d iov=%p iovcnt=%d offset=%lld",
          fd, iov, iovcnt, static_cast<int64_t>(offset));
  ALOG_ASSERT(0);
  errno = ENOSYS;
  ARC_STRACE_RETURN(-1);
...
extern "C" ssize_t wrap_pwritev_func(int fd, const struct iovec* iov,
                                     int iovcnt, off_t offset) {
  ARC_STRACE_ENTER_FD("writev", "%d, %p, %d, %lld",
                        fd, iov, iovcnt, static_cast<int64_t>(offset));
  DANGERF("pwritev: fd=%d iov=%p iovcnt=%d offset=%lld",
          fd, iov, iovcnt, static_cast<int64_t>(offset));
  ALOG_ASSERT(0);
  errno = ENOSYS;
  ARC_STRACE_RETURN(-1);
...
extern "C" int __wrap_inotify_init1(int flags) {
  ARC_STRACE_ENTER("inotify_init1", "%d", flags);
  // TODO(crbug.com/236903): Implement this.
  DANGERF("inotify_init1: flags=%d", flags);
  errno = ENOSYS;
  ARC_STRACE_RETURN(-1);
}
