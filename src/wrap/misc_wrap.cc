...
extern "C" {
#define wrap_getrlimit_func __wrap_getrlimit
#define wrap_setrlimit_func __wrap_setrlimit

void __wrap_abort();
void __wrap_exit(int status);
int __wrap_fork();
int __wrap_getpriority(int which, int who);
int wrap_getrlimit_func(int resource, struct rlimit *rlim);
...
int wrap_setrlimit_func(int resource, const struct rlimit *rlim);
...
extern int __real_pthread_kill(pthread_t thread, int sig);
}  // extern "C"
...
int __wrap_setpriority(int which, int who, int prio) {
  ...
  if (which == PRIO_PROCESS) {
    ...
    if (who == -1) {
      // TODO(yusukes): This is for 64-bit nacl-glibc target which we removed
      // long ago. Remove the if block.
      errno = ESRCH;
      ARC_STRACE_RETURN(-1);
    }
