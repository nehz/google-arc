...
extern "C" {
#define wrap_creat_func __wrap_creat
#define real_mkostemp_func __real_mkostemp
#define wrap_mkostemp_func __wrap_mkostemp
#define real_mkostemps_func __real_mkostemps
#define wrap_mkostemps_func __wrap_mkostemps
#define real_mkstemp_func __real_mkstemp
#define wrap_mkstemp_func __wrap_mkstemp
#define real_mkstemps_func __real_mkstemps
#define wrap_mkstemps_func __wrap_mkstemps
#define real_mmap_func __real_mmap
#define wrap_mmap_func __wrap_mmap
#define real_open_func __real_open
#define wrap_open_func __wrap_open
#define real_statfs_func __real_statfs
#define wrap_statfs_func __wrap_statfs
#define real_statvfs_func __real_statvfs
#define wrap_statvfs_func __wrap_statvfs
#define wrap_tmpfile_func __wrap_tmpfile
...
int wrap_mkostemp_func(char* tmpl, int flags);
int wrap_mkostemps_func(char* tmpl, int suffix_len, int flags);
int wrap_mkstemp_func(char* tmpl);
int wrap_mkstemps_func(char* tmpl, int suffix_len);
int wrap_open_func(const char* pathname, int flags, ...);
...
int wrap_stat_func(const char* filename, struct stat* buf);
int wrap_statfs_func(const char* path, struct statfs* stat);
int wrap_statvfs_func(const char* path, struct statvfs* stat);
FILE* wrap_tmpfile_func();
...
extern int real_mkostemp_func(char* tmpl, int flags);
extern int real_mkostemps_func(char* tmpl, int suffix_len, int flags);
extern int real_mkstemp_func(char* tmpl);
extern int real_mkstemps_func(char* tmpl, int suffix_len);
extern int real_open_func(const char* pathname, int flags, mode_t mode);
...
extern int real_stat_func(const char* filename, struct stat* buf);
extern int real_statfs_func(const char* filename, struct statfs* buf);
extern int real_statvfs_func(const char* filename, struct statvfs* buf);
...
int wrap_creat_func(const char* pathname, mode_t mode);
...
int __wrap_ppoll(struct pollfd* fds, nfds_t nfds, int timeout,
...
extern int __real_flock(int fd, int operation);
...
extern int __real_mlockall(int flags);
extern void* real_mmap_func(
    void* addr, size_t length, int prot, int flags, int fd, off_t offset);
...
// as-is like opendir().
int wrap_mkostemps_func(char* tmpl, int suffix_len, int flags) {
  ARC_STRACE_ENTER("mkostemps", "\"%s\", %d, %s",
                   SAFE_CSTR(tmpl), suffix_len,
                   arc::GetOpenFlagStr(flags).c_str());
...
int wrap_mkostemp_func(char* tmpl, int flags) {
  ARC_STRACE_ENTER("mkostemp", "\"%s\" %s",
                    tmpl, arc::GetOpenFlagStr(flags).c_str());
  int fd = wrap_mkostemps_func(tmpl, flags, 0);
  ARC_STRACE_RETURN(fd);
}
...
int wrap_mkstemp_func(char* tmpl) {
  ...
  int fd = wrap_mkostemp_func(tmpl, 0);
  ARC_STRACE_RETURN(fd);
}

int wrap_mkstemps_func(char* tmpl, int suffix_len) {
  ARC_STRACE_ENTER("mkstemps", "\"%s\" %d", SAFE_CSTR(tmpl), suffix_len);
  int fd = wrap_mkostemps_func(tmpl, suffix_len, 0);
  ARC_STRACE_RETURN(fd);
}
...
int wrap_open_func(const char* pathname, int flags, ...) {
  ...
  if (file_system)
    ...
    fd = real_open_func(pathname, flags, mode);
...
int wrap_statfs_func(const char* pathname, struct statfs* stat) {
  ...
  if (file_system)
    ...
    result = real_statfs_func(pathname, stat);
  if (result == -1 && errno != ENOENT)
    DANGERF("path=%s: %s", SAFE_CSTR(pathname), safe_strerror(errno).c_str());
  ARC_STRACE_REPORT(
...
int wrap_statvfs_func(const char* pathname, struct statvfs* stat) {
  ...
  if (file_system)
    ...
    result = real_statvfs_func(pathname, stat);
  ARC_STRACE_REPORT(
...
void* wrap_mmap_func(
  ...
  if (file_system)
    ...
    result = real_mmap_func(addr, length, prot, flags, fd, offset);
...
// TODO(yusukes): This function used to be used in Glibc mode by NDK
// trampolines, but is no longer necessary. Remove.
ssize_t __wrap_pread(int fd, void* buf, size_t count, off_t offset) {
...
IRT_WRAPPER(mmap, void **addr, size_t len, int prot, int flags,
            int fd, nacl_abi_off_t off) {
  ALOG_ASSERT(__nacl_irt_mmap_real);
  const int result = __nacl_irt_mmap_real(addr, len, prot, flags, fd, off);

#if defined(MMAP_IRT_LOGGING)
  // Dump all the mmap/munmap IRT calls except ones in the Bionic loader. To see
  // DSO mapping details, use --logging=bionic-loader instead. For more details,
  // see the comments for DO_WRAP above.

  // Since calling a function which may allocate memory here is not safe and
  // might cause a deadlock, use base::string::SafeSNPrintf instead of usual
  // ones like snprintf and base::StringPrintf. Use the '[[arc_strace]]:'
  // prefix so that the log goes to arc_strace.log when
  // --enable-arc-strace is also specified.
  if (!result) {
    char str[512];
    base::strings::SafeSNPrintf(str, sizeof(str),
                                "[[arc_strace]]:   mmap IRT: (%p-%p) %s\n",
                                *addr, static_cast<char*>(*addr) + len,
                                // We do not use GetMmapFlagsStr here since
                                // it also allocates memory.
                                (flags & MAP_FIXED) ? "FIXED" : "");
    size_t nwrote = 0;
    __nacl_irt_write_real(STDERR_FILENO, str, strlen(str), &nwrote);
  }
#endif  // MMAP_IRT_LOGGING

  return result;
}

IRT_WRAPPER(munmap, void* addr, size_t len) {
  ALOG_ASSERT(__nacl_irt_munmap_real);
  const int result = __nacl_irt_munmap_real(addr, len);

#if defined(MMAP_IRT_LOGGING)
  if (!result) {
    char str[512];
    base::strings::SafeSNPrintf(str, sizeof(str),
                                "[[arc_strace]]: munmap IRT: (%p-%p)\n",
                                addr, static_cast<char*>(addr) + len);
    size_t nwrote = 0;
    __nacl_irt_write_real(STDERR_FILENO, str, strlen(str), &nwrote);
  }
...
void* __real_mmap_nohook(void* addr, size_t length, int prot, int flags, int fd,
                         off_t offset) {
  // It is safe to pass Linux's |prot| and |flags| to NaCl as-is.
  const int result =
      __nacl_irt_mmap_real(&addr, length, prot, flags, fd, offset);
  if (result != 0) {
    errno = result;
    return MAP_FAILED;
  }
...
int __real_munmap_nohook(void* addr, size_t length) {
  const int result = __nacl_irt_munmap_real(addr, length);
  if (result != 0) {
    errno = result;
    return -1;
  }
...
void InitIRTHooks() {
  ...
  DO_WRAP(seek);
  DO_WRAP(write);
  // Unlike others, mmap and munmap are also -Wl,--wrap'ped in addition to the
  // IRT hooks. Link-time wrappers are for redirecting mmap/munmap calls to
  // posix_translation and IRT hooks are for debug logging. Note that it is not
  // a good idea to call into posix_translation from IRT hooks because the hooks
  // are called while dlmalloc's internal mutex is locked (see
  // http://crbug.com/339717#c6 ).
  DO_WRAP(mmap);
  DO_WRAP(munmap);
...
const LibcDispatchTable g_libc_dispatch_table = {
  ...
  __real_lseek64,
  __real_mmap_nohook,  // both IRT hook and --wrap
  __real_mprotect,
  __real_munmap_nohook,  // both IRT hook and --wrap
  __real_open,
  __real_read,
  __real_write,
};
