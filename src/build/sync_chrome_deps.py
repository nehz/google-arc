...
def main():
  ...
  if len(chrome_hash) != 40:
    print 'Chrome deps must be a Git hash'
    return 1
  ...
  with open(_PPAPI_DIR + '/.DEPS.git') as f:
    ppapi_deps = f.read().replace('\n', ' ')
  m = re.match(
      r'.*native_client/src/native_client\.git@([0-9,a-f]+).*', ppapi_deps)
  if not m:
