...
def _check_wrapper_functions_are_defined(functions, libppapi_arc):
  libppapi_arc_funcs = _get_defined_functions(libppapi_arc)
  wrapper_funcs = set()
  for func in libppapi_arc_funcs:
  ...
  for func in sorted(functions - wrapper_funcs):
    ...
    print ('--wrap=%s is specified in wrapped_functions.py, '
           'but __wrap_%s is not defined in libppapi_arc.so' % (func, func))
  ...
  for func in sorted(wrapper_funcs - functions - whitelist):
    ...
    print ('--wrap=%s is not specified in wrapped_functions.py, '
           'but __wrap_%s is defined in libppapi_arc.so' % (func, func))
...
def main():
  ...
  if len(sys.argv) < 3:
    print 'Usage: %s libppapi_arc.so libc.so...'
    return 1

  libppapi_arc = sys.argv[1]
  libc_libraries = sys.argv[2:]
  functions = set(wrapped_functions.get_wrapped_functions())

  ok = _check_wrapper_functions_are_defined(functions, libppapi_arc)
