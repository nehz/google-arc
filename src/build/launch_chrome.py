...
from util.output_handler import TestBootHandler
...
def _write_preferences_if_needed():
  # Copy in a Preferences file that whitelists libppapi_arc.
  pref_dest = os.path.join(_USER_DATA_DIR, 'Default', 'Preferences')
  if not os.path.exists(pref_dest):
    build_common.makedirs_safely(os.path.dirname(pref_dest))
    pref_file = os.path.join(_ROOT_DIR, 'src', 'build', 'arc_chrome_prefs')
    shutil.copy2(pref_file, pref_dest)
...
def _is_no_sandbox_needed(parsed_args):
  ...
  # Chrome tracing calls clock_gettime with a Chrome OS only clock
  # ID. Temporary disables sandbox to workaround this issue.
  # TODO(crbug.com/378063): Remove this.
  if parsed_args.tracestartup > 0 and OPTIONS.is_arm():
    return False
...
def _compute_chrome_performance_test_params(unused_parsed_args):
  """Add params that are necessary for stable testboot result."""
  params = []
...
def _compute_chrome_params(parsed_args):
  ...
  if parsed_args.mode == 'testboot':
    # Do not show the New Tab Page because showing NTP during testboot makes the
  ...
  if parsed_args.mode == 'testboot' or parsed_args.mode == 'atftest':
    # Make the window size small on Goobuntu so that it does not cover the whole
    # desktop during testboot/integration_test.
...
def _should_timeouts_be_used(parsed_args):
  ...
  if parsed_args.mode not in ('atftest', 'testboot'):
    return False
...
def _select_output_handler(parsed_args, stats, chrome_process, **kwargs):
  ...
  elif parsed_args.mode == 'testboot':
    output_handler = TestBootHandler(parsed_args, stats, chrome_process,
                                     **kwargs)
  else:
  ...
  if 'gpu' in parsed_args.gdb or 'renderer' in parsed_args.gdb:
    output_handler = gdb_util.GdbHandlerAdapter(
        output_handler, parsed_args.gdb)
