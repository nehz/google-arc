...
def _validate_testboot_settings(parser, args):
  if args.mode == 'testboot':
    if args.perf_test and args.iterations < 1:
      parser.error("--perf-test must be run with --iterations")
  else:
    if args.no_cache_warming:
      parser.error("--no-cache-warming only valid in 'testboot' mode")
    if args.iterations:
      parser.error("--iterations only valid in 'testboot' mode")
    if args.minimum_lifetime:
      parser.error("--minimum-lifetime only valid in 'testboot' mode")
    if args.minimum_steady:
      parser.error("--minimum-steady only valid in 'testboot' mode")
...
def _validate_timeout(parser, args):
  if (args.timeout != _DEFAULT_TIMEOUT and
      args.mode not in ['testboot', 'atftest', 'system']):
    parser.error("--timeout  only valid in 'testboot' or 'atftest' mode")
...
def _resolve_remote_execution_mode(args):
  ...
  # where disk is very fast and many CPU cores are available, the tasks running
  # in the browser process does not affect testboot score that much, but on
  # Chromebooks, especially Yoshi, it does. To make the testboot score more
  # realistic, add 1.5s wait to the onLaunched handler in main.html.
  if (args.minimum_launch_delay is None and args.mode == 'testboot' and
      args.remote):
...
""", formatter_class=argparse.RawTextHelpFormatter)
  ...
  parser.add_argument('--no-cache-warming', action='store_true',
                      help='Works with testboot command only. Not starts the '
  ...
  parser.add_argument('--gdb-type', choices=('xterm', 'wait'),
  ...
  parser.add_argument('--iterations', type=int, metavar='<N>', help='Works '
                      'with testboot command only.  Starts the plugin page <N> '
  ...
  parser.add_argument('--minimum-lifetime', type=int, default=0,
                      metavar='<T>', help='Works with testboot only. '
  ...
  parser.add_argument('--output-timeout', type=int, default=None,
                      ...
                      'testboot commands only.  Specifies the timeout in '
  ...
  parser.add_argument('--perf-test', action='store_true', help=
                      'Runs testboot with performance test specific options.')
  ...
  parser.add_argument('--timeout', type=int, default=_DEFAULT_TIMEOUT,
                      metavar='<T>', help='Works with atftest, system and '
                      'testboot commands only.  Specifies timeout for running '
  ...
  _validate_testboot_settings(parser, args)
  ...
  _resolve_remote_execution_mode(args)
