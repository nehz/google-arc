...
CHECKED_LIBRARIES = ['libppapi_arc.so', 'libposix_translation.so']
...
def is_launch_chrome_command(args):
  if args[0] == '/bin/sh':
    args = args[1:]
...
def remove_leading_launch_chrome_args(argv):
  ...
  while is_launch_chrome_command(argv):
    argv = argv[1:]
  return argv
...
def get_build_path_for_executable(executable, is_host=False):
  ...
  target: Not supported but 'is_host' parameter exists for consistency.
  """
  assert is_host  # Target executable should not exist.
  return os.path.join(get_build_dir(is_host=is_host),
...
def get_chrome_revision_by_hash(hash):
  ...
  m = re.match(r'.*git-svn-id:.*@([0-9]+).*', message)
  if m:
    return m.group(1)
...
def get_runtime_file_list_header():
  return os.path.join(get_build_dir(), 'runtime_file_list.h')
