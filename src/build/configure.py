...
def _verify_ninja_generator_list(ninja_list):
  ...
  for ninja in ninja_list:
    ...
    if (isinstance(ninja, ninja_generator.ExecNinjaGenerator) and
        # Do not check the used count of tests and the main nexe. Note
        # we link most target archives to both the main nexe and
        # libppapi_arc.so.
        not isinstance(ninja, ninja_generator.TestNinjaGenerator) and
        ninja.get_module_name() != 'arc_%s.nexe' % OPTIONS.target()):
