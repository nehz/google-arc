...
_UNIT_TEST_FILE_PATTERNS = ['out/target/%(target)s/lib',
                            'out/target/%(target)s/test_info']


# Flags to launch Chrome on remote host.
_REMOTE_FLAGS = ['--nacl-helper-binary', '--remote',
                 '--ssh-key', '--remote-host-type']
...
def create_launch_remote_chrome_param(argv):
  ...
  - Removes --remote, --ssh-key and --remote-host-type flags.
  - Adds --noninja flag.
  """
  result_argv = []
  skip_next = False
