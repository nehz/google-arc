...
def _get_app_res_mem(pid):
  line = _get_process_stat_line(pid)
  return int(line.split()[23]) * 4096.0 / 1024 / 1024
...
def _get_nacl_arc_process_resident_memory(chrome_pid):
  # Take some extreme measures to get the resident memory usage of nacl_helper.
  # From the untrusted code we cannot know the amount of resident memory used,
  ...
  # running ARC.  Note that this is the absolute amount of resident memory.
  ...
  if len(pids) not in (2, 3):
    return None
  app_res_mem = max([_get_app_res_mem(pid) for pid in pids])
  sys.stderr.write('NaCl ARC process resident memory: %dMB\n' % app_res_mem)
  return app_res_mem
  ...
  def handle_stderr(self, line):
    ...
    if self.stats.parse_app_start_message(line):
      ...
      if platform_util.is_running_on_linux():
        app_res_mem = \
            _get_nacl_arc_process_resident_memory(self.chrome_process.pid)
        if app_res_mem is not None:
          self.stats.app_res_mem = app_res_mem
      sys.stderr.write(dash_line + '\n')
      ...
      if self.parsed_args.minimum_lifetime:
        ...
        return
      if self.parsed_args.perf_test and self.cache_warming:
...
class ArcStraceFilter(object):
  ...
  def handle_stderr(self, line):
    ...
    if matched:
      ...
      # Keep leading part if necessary. Note that the traling LF is also
