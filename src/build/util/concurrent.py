...
def _broker_thread_run(
  ...
  while True:
    if send_sentinel and not future_dict:
      # Sentinels were sent to all the workes, and all running tasks are
      # completed.
      break
...
class CancelledError(Exception):
  """Raised when Future.result() is called for cancelled insntace."""
  pass
...
class Future(object):
  ...
  def _take_done_callback_list(self):
    ...
    This function must be called while the |_cond| is aquired.
    """
