...
# The path of the chrome executable on Chrome OS.
_CHROMEOS_CHROME_EXE_PATH = '/opt/google/chrome/chrome'
...
def get_chrome_exe_path_on_remote_host():
...
# NOTE: Prefer using get_chrome_exe_path over get_chrome_exe_path_on_* so that
# the script uses the proper chrome exe path both on remote and local hosts
# unless you are absolutely sure that the script runs on either local or remote
# host.
def get_chrome_exe_path():
  ...
  if platform_util.is_running_on_remote_host():
    return get_chrome_exe_path_on_remote_host()
...
def add_remote_arguments(parser):
  parser.add_argument('--remote', help='The IP address of the remote host, '
                      ...
                      'image, or cygwin\'s sshd on Windows.')
  ...
  parser.add_argument('--remote-host-type',
                      choices=('chromeos', 'cygwin', 'mac'), default='chromeos',
                      help='The OS type of the remote host. Used only when '
                      '--remote option is specified.')
...
def launch_remote_chrome(parsed_args, argv):
  ...
  if parsed_args.remote_host_type == 'cygwin':
    return remote_windows_executor.launch_remote_chrome(parsed_args, argv)
  if parsed_args.remote_host_type == 'mac':
