...
def maybe_launch_gdb(gdb_target_list, gdb_type, chrome_pid):
  ...
  if 'browser' in gdb_target_list:
    _launch_gdb('browser', str(chrome_pid))
...
def _launch_gdb(title, pid_string):
  """Launches GDB for a non-plugin process."""
  command = _get_xterm_gdb_startup(title, toolchain.get_tool('host', 'gdb'))
...
class GdbHandlerAdapter(object):
  ...
  def __init__(self, base_handler, target_list):
