...
def _copy_adb_to_dir_without_noexec(executor):
  """Returns the command to copy the adb directory for testing.

  We copy the adb directory to a directory whose filesystem is mounted
  without noexec mount option, so that the adb can be executed directly.
  This returns the command for it.
  """
  source = os.path.join(executor.get_remote_arc_root(), 'out/adb')
  destination = os.path.join(toolchain.get_chromeos_dir_without_noexec(),
                             'out')
...
def launch_remote_chrome(parsed_args, argv):
  try:
    ...
    command = ' '.join(
        _copy_adb_to_dir_without_noexec(executor) +
...
def extend_chrome_params(parsed_args, params):
  ...
  new_params.remove('--login-manager')
  # Append the flags that are not set by our scripts.
  for new_param in new_params:
    if not _is_param_set(new_param, params):
...
def run_remote_unittest(parsed_args, copied_files):
  ...
  try:
    ...
    command = ' '.join(
        ['cd', executor.get_remote_arc_root(), '&&',
         executor.get_remote_env(),
         remote_executor_util.RUN_UNIT_TEST] + verbose + parsed_args.tests)
...
def cleanup_remote_files(parsed_args):
  ...
  removed_patterns = [
      ...
      # The directory adb is copied to.
      os.path.join(toolchain.get_chromeos_dir_without_noexec(), 'out'),
  ...
  executor.run(' '.join(['rm', '-rf'] + removed_patterns))
