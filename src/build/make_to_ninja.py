...
def prepare_make_to_ninja():
  ...
  # Webkit references libstlport.mk which we don't need.
  with _open_for_write('external/stlport/libstlport.mk') as f:
    f.write('\n')
...
class MakeVars:
  ...
  def is_clang_ready(self):
    assert self.is_c_library() or self.is_executable()
    return self._is_clang_ready
  ...
  def set_is_clang_ready(self, ready):
    assert self.is_c_library() or self.is_executable()
    self._is_clang_ready = ready
  ...
  def use_logtag_emission(self):
    return self._emit_logtag

  def disable_logtag_emission(self):
    self._emit_logtag = False
...
def _generate_for_shared_or_executable(vars, n, copied_headers):
  ...
  if vars.is_host_shared():
    ...
    # which will be resolved when linking libppapi_arc.so).
...
def _generate_c_ninja(vars, out_lib_deps):
  ...
  extra_args['emit_logtag'] = vars.use_logtag_emission()
  extra_args['extra_notices'] = vars.get_extra_notices()
  extra_args['is_clang_ready'] = vars.is_clang_ready()
