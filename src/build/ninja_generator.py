  ...
  def emit_common_rules(n):
    ...
    n.rule('verify_disallowed_symbols',
           ('src/build/symbol_tool.py --verify $in $disallowed_symbols && '
            'touch $out'),
           description='verify_disallowed_symbols $in')
...
class CNinjaGenerator(NinjaGenerator):
  ...
  def __init__(self, module_name, ninja_name=None, emit_logtag=True,
               gl_flags=False, is_clang_ready=False, **kwargs):
    ...
    self._use_clang = (is_clang_ready and
...
class TestNinjaGenerator(ExecNinjaGenerator):
  ...
  @staticmethod
  def emit_common_rules(n):
    n.variable('runner', toolchain.get_tool(OPTIONS.target(), 'runner'))
    n.variable('valgrind_runner',
               toolchain.get_tool(OPTIONS.target(), 'valgrind_runner'))

    n.rule('run_test',
           '$runner $in $argv %s' % build_common.get_test_output_handler(),
           description='run_test $in')
    # NOTE: When $runner is empty, there will be an extra space in front of the
    # test invocation.
    n.rule('run_gtest',
           ('$runner $in $argv --gtest_color=yes' +
            build_common.get_test_output_handler(use_crash_analyzer=True)),
           description='run_gtest $in')
    if OPTIONS.is_bare_metal_build():
      runner = ''
      if OPTIONS.is_arm():
        runner = ' '.join(toolchain.get_qemu_arm_args()) + ' '
      n.rule('run_gtest_glibc',
             (runner + '$in $argv --gtest_color=yes' +
              build_common.get_test_output_handler(use_crash_analyzer=True)),
             description='run_gtest_glibc $in')
    n.rule('run_gtest_with_valgrind',
           ('$valgrind_runner $in $argv --gtest_color=yes' +
            build_common.get_test_output_handler()),
           description='run_gtest_with_valgrind $in')
...
class JavaNinjaGenerator(NinjaGenerator):
  ...
  def _build_and_add_all_generated_sources(self):
    self.build_and_add_logtags()
    self.build_and_add_aidl_generated_java()

  def build_and_add_resources(self):
    ...
    self._build_aapt(outputs=java_files, implicit=resource_files,
  ...
  def build_all_added_sources(self):
...
class ApkNinjaGenerator(JavaNinjaGenerator):
  ...
  def _get_minimal_bootclasspath(self):
    ...
    a -bootclasspath option, the Java compiler would instead consult its
    built-in core libraries, whereas Android uses a separate Apache
    core library (as well as API frameworks, etc).  APKs should
    only have access to /system/framework/framework.jar and things in its
    bootclasspath.
    """
  ...
  def emit_common_rules(n):
    ...
    n.rule('run_python_test',
           ('PYTHONPATH=third_party/tools/python_mock python -m '
            'unittest discover $test_path $test_name $top_path' +
            build_common.get_test_output_handler()),
