...
// Logger class
Logger::Logger() {
  buffers_[ARC_LOG_ID_MAIN].reset(new LoggerBuffer(kLoggerBufferSize));
  buffers_[ARC_LOG_ID_RADIO].reset(new LoggerBuffer(kLoggerBufferSize));
  buffers_[ARC_LOG_ID_EVENTS].reset(new LoggerBuffer(kEventLoggerBufferSize));
  buffers_[ARC_LOG_ID_SYSTEM].reset(new LoggerBuffer(kLoggerBufferSize));
}
...
void Logger::WaitForReadReady(LoggerReader* reader,
    const base::Callback<void(void)>& callback) {
  return reader->buffer()->WaitForReadReady(reader, callback);
}
