...
void Trace(const char* format, ...) {
  // ARC MOD BEGIN
  // Workaround for https://code.google.com/p/nativeclient/issues/detail?id=3844
  // TODO(nativeclient:3844): Remove this workaround if nativeclient:3844 is
  // fixed or we switch the toolchain to clang.
  va_list va;
  va_start(va, format);
  va_end(va);
  // ARC MOD END
