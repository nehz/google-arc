...
// ARC MOD BEGIN
// For unit test, instead of depending on ANDROID_DATA env var, use
// the ARC build system's path passed on via -D.
#if !defined(ARC_TARGET_PATH)
// ARC MOD END
#if defined(__LP64__)
...
// ARC MOD BEGIN
#else
#define LIBPATH "%s/lib/libdlext_test_fd.so"
#endif
// ARC MOD END
...
TEST_F(DlExtTest, ExtInfoUseFd) {
  ...
  // the ARC build system's path passed on via -D.
#else
  const char* android_data = ARC_TARGET_PATH;
#endif
  // ARC MOD END
...
TEST_F(DlExtTest, Reserved) {
  ...
  fn f = reinterpret_cast<fn>(dlsym(handle_, "getRandomNumber"));
  ASSERT_DL_NOTNULL(f);
  // ARC MOD BEGIN UPSTREAM dlext-test-pointer-cast-for-clang
...
protected:
  virtual void SetUp() {
    ...
    // ARC MOD BEGIN
    // Use our temp path instead of ANDROID_DATA for ARC bionic unittest.
#if !defined(HAVE_ARC)
    // ARC MOD END
    const char* android_data = getenv("ANDROID_DATA");
    ASSERT_TRUE(android_data != NULL);
    ...
    // ARC MOD BEGIN
    // Use a temp path instead of ANDROID_DATA for ARC bionic unittest.
#else
    strlcpy(relro_file_, "out/tmp/libdlext_test.relro", sizeof(relro_file_));
    // Try creating out/tmp directory if it does not exist.
    if (-1 == mkdir("out/tmp", S_IRUSR | S_IWUSR | S_IXUSR)) {
