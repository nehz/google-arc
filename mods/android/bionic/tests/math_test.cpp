...
TEST(math, __fpclassifyl) {
  ...
  // __fpclassifyl must not delegate to double version because
  // cast may change a finite value to infinite.
  EXPECT_EQ(FP_NORMAL, __fpclassifyl(LDBL_MAX));
  // ARC MOD END
#else // __BIONIC__
  GTEST_LOG_(INFO) << "This test does nothing.\n";
#endif // __BIONIC__
...
TEST(math, __isfinitel) {
  ...
  // ARC MOD BEGIN bionic-long-double
  // __isfinitel must not delegate to double version because
  // cast may change a finite value to infinite.
  ASSERT_TRUE(__isfinitel(LDBL_MAX));
  // ARC MOD END
#else // __BIONIC__
  GTEST_LOG_(INFO) << "This test does nothing.\n";
#endif // __BIONIC__
...
TEST(math, __isinfl) {
  ...
  // ARC MOD BEGIN bionic-long-double
  // __isinfl must not delegate to double version because
  // cast may change a finite value to infinite.
  ASSERT_FALSE(__isinfl(LDBL_MAX));
  // ARC MOD END
}
