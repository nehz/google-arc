...
/* ARC MOD BEGIN */
// Allow to set own write_to_log handler. Used to hook writing to log in ARC
// in order to simulate multi-process environment.
#if !FAKE_LOG_DEVICE
typedef int (*write_to_log_func)(log_id_t, struct iovec *, size_t);

int set_write_to_log(write_to_log_func func) {
    // Make sure there were no previous calls to log.
    if (logd_fd != -1 || write_to_log != __write_to_log_init)
      return -1;
    write_to_log = func;
    return 0;
}
...
// ARC MOD BEGIN
// ARC has own version of __android_log_assert available for target. In host
// mode FAKE_LOG_DEVICE is defined.
#if FAKE_LOG_DEVICE
// ARC MOD END
