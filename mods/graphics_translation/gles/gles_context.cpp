...
void GlesContext::DrawTex(GLfloat x, GLfloat y, GLfloat z, GLfloat width,
  ...
  const arc::Matrix projection_matrix = uniform_context_.GetProjectionMatrix();
  const GLint* viewport = viewport_.Get();
  uniform_context_.MutateProjectionMatrix() = arc::Matrix::GenerateOrthographic(
      viewport[0], viewport[0] + viewport[2], viewport[1],
      viewport[1] + viewport[3], 0.f, -1.f);

  // Setup modelview matrix
  const arc::Matrix model_view_matrix = uniform_context_.GetModelViewMatrix();
  uniform_context_.MutateModelViewMatrix().AssignIdentity();

  // Setup texture matrix
  arc::Matrix texture_matrices[UniformContext::kMaxTextureUnits];
  ...
  // Restore pointers
  for (GLint index = 0; index < pointers.size(); ++index) {
...
bool GlesContext::GetValue(GLenum value, T* data) const {
  switch (value) {
    ...
    case GL_NUM_COMPRESSED_TEXTURE_FORMATS:
      EnsureCompressedTextureFormatStateKnown();
      Convert(data, supported_compressed_texture_formats_.size());
      return true;
