...
#include "graphics_translation/gles/state.h"
...
UniformContext::UniformContext(GlesContext* context)
    : context_(context),
      ...
      ambient_(arc::Vector(0.2f, 0.2f, 0.2f, 1.f)),
      color_(arc::Vector(1.f, 1.f, 1.f, 1.f)),
      normal_(arc::Vector(0.f, 0.f, 1.f, 0.f)) {
  ...
  // es_full_spec.1.1.12.pdf section 2.12.1 table 2.8.
  lights_[0].Mutate().diffuse = arc::Vector(1.f, 1.f, 1.f, 1.f);
  lights_[0].Mutate().specular = arc::Vector(1.f, 1.f, 1.f, 1.f);
}
...
void UniformContext::Bind(ProgramContext* program) {
  ...
  if (force_update || mv_is_dirty) {
    const arc::Matrix& model_view_matrix =
    ...
    if (program->GetUniformLocation(
      ...
      arc::Matrix mv_inverse_transpose = model_view_matrix;
  ...
  if (force_update || p_is_dirty) {
    const arc::Matrix& projection_matrix =
  ...
  for (int i = 0; i < kMaxTextureUnits; ++i) {
    ...
    if (force_update || texture_matrix_stack_[i].IsDirty()) {
      const arc::Matrix& texture_matrix =
  ...
  for (int i = 0; i < kMaxPaletteMatricesOES; ++i) {
    if (force_update || palette_matrices_[i].IsDirty()) {
      arc::Matrix matrix = palette_matrices_[i].Get();
...
const arc::Matrix& UniformContext::GetModelViewMatrix() const {
  return modelview_matrix_stack_.Get().GetTop();
}

arc::Matrix& UniformContext::MutateModelViewMatrix() {
  return modelview_matrix_stack_.Mutate().GetTop();
}

const arc::Matrix& UniformContext::GetProjectionMatrix() const {
  return projection_matrix_stack_.Get().GetTop();
}

arc::Matrix& UniformContext::MutateProjectionMatrix() {
  return projection_matrix_stack_.Mutate().GetTop();
}

const arc::Matrix& UniformContext::GetTextureMatrix() const {
  return texture_matrix_stack_[active_texture_unit_].Get().GetTop();
}

const arc::Matrix& UniformContext::GetTextureMatrixByStage(GLint stage) const {
  LOG_ALWAYS_FATAL_IF(stage < 0 || stage >= kMaxTextureUnits);
  return texture_matrix_stack_[stage].Get().GetTop();
}

arc::Matrix& UniformContext::MutateTextureMatrixByStage(GLint stage) {
  LOG_ALWAYS_FATAL_IF(stage < 0 || stage >= kMaxTextureUnits);
...
arc::Matrix& UniformContext::MutateActiveMatrix() {
  if (matrix_mode_ == GL_MATRIX_PALETTE_OES) {
...
const arc::Matrix& UniformContext::GetActiveMatrix() const {
  if (matrix_mode_ == GL_MATRIX_PALETTE_OES) {
...
const arc::Matrix& UniformContext::GetPaletteMatrix() const {
  return palette_matrices_[current_palette_matrix_].Get();
}

arc::Matrix& UniformContext::MutatePaletteMatrix() {
...
arc::Vector* UniformContext::MutateClipPlane(GLenum id) {
...
const arc::Vector* UniformContext::GetClipPlane(GLenum id) const {
  const int index = id - GL_CLIP_PLANE0;
  if (index >= 0 && index < kMaxClipPlanes) {
...
void ProgramContext::BindUniform(const arc::Vector& vector, UniformKey key) {
...
void ProgramContext::BindUniform(const arc::Vector& vector, UniformKey key,
  ...
  const GLint location = GetUniformLocation(key, index);
  if (location != -1) {
    GLfloat float_array[arc::Vector::kEntries];
    vector.GetFloatArray(float_array);
    PASS_THROUGH(context_, Uniform4fv, location, 1, float_array);
  }
}

void ProgramContext::BindUniform(const arc::Matrix& matrix, UniformKey key) {
...
void ProgramContext::BindUniform(const arc::Matrix& matrix, UniformKey key,
  ...
  const GLint location = GetUniformLocation(key, index);
  if (location != -1) {
    GLfloat float_array[arc::Matrix::kEntries];
...
void ProgramContext::BindUniform(const Light& light, int index) {
  const float light_spot_cutoff =
      cos(light.spot_cutoff * arc::kRadiansPerDegree);
  arc::Vector light_position = light.position;
  if (light_position.Get(3) == 0.f) {
    light_position.Normalize();
  }
  arc::Vector light_direction = light.direction;
...
void ProgramContext::BindUniform(const TexEnv& texenv, int index) {
  ...
  const arc::Vector scale = arc::Vector(texenv.rgb_scale, texenv.rgb_scale,
